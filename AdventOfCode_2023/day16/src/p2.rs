...
fn non_recursive_beam_path(tiles:&Matrix<Tile>,seen:&mut HashMap<usize,Vec<Direction>>,initial_idx:usize, initial_dir:Direction){
...
fn beam_path(tiles: &Matrix<Tile>, start_idx:usize, start_dir:Direction)->usize{
    let mut seen = HashMap::<usize,Vec<Direction>>::new();
    // recursive_beam_path(0, Direction::East, &tiles, &mut seen);
    non_recursive_beam_path(tiles,&mut seen,start_idx, start_dir);
    #[cfg(test)]{
        println!();
...
pub fn main_2(file_name:&str)->Option<usize>{
    ...
    let mut edge_cases = Vec::<(usize,Direction)>::with_capacity( 2*(tiles.xmax + tiles.ymax) );
    for (x,dir) in vec![(0, Direction::South),(tiles.xmax-1, Direction::North)].iter(){
        for y in 0..tiles.ymax{
            let idx = tiles.coord_to_idx([*x,y]);
            edge_cases.push((idx,*dir));
        }
    }
    for (y,dir) in vec![(0,Direction::East),(tiles.ymax-1,Direction::West)].iter(){
        for x in 0..tiles.xmax{
            let idx = tiles.coord_to_idx([x,*y]);
            edge_cases.push((idx,*dir));
    ...
    for (idx, dir) in edge_cases.into_iter(){
        let total_energized = beam_path(&tiles,idx,dir);
        if max_energized < total_energized{
            max_energized = total_energized;
        }
    }
    Some(max_energized)
