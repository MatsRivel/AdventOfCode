...
pub fn main_2(file_name:&str)->Option<String>{
    ...
    let mut known_allergens = allergens.into_iter().filter_map(|(allergen, value)| Some((allergen, value.get_defined_value().unwrap())) ).collect::<Vec<(Allergen,Food)>>();
    known_allergens.sort();
    let output = known_allergens.into_iter().map(|(_,f)| f).collect::<Vec<String>>().join(",");
    Some(output)
